    ...
    execute: function () {
      ...
      _export("SpellingController", SpellingController = (_dec = ccclass('SpellingController'), _dec2 = property({
        ...
        buildSpelling(config) {
          this.levelsData = [];
          var levels = config.split('\n');

          for (var i = 0; i < levels.length; i++) {
            var level = new (_crd && SpellingData === void 0 ? (_reportPossibleCrUseOfSpellingData({
              error: Error()
        ...
        applyLevelData() {
          var data = this.levelsData[this.currentLevelIndex];
        ...
        onSomeVariantSelected(event) {
          ...
          var spellingTweens = this.spellingView.uncoverWord(this.levelsData[this.currentLevelIndex].getCorrectWord());
          var correctLabelTween = this.correctlabel.show();
          var allTweens = spellingTweens;
        ...
        showNextLevel() {
          var changeLevelDelay = this.animation.getState(this.changeLevelClipName).clip.duration / 2;
...
//# sourceMappingURL=07b92929958931b1a83d29c8664fc85f39f28f56.js.map