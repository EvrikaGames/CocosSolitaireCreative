  ...
  return {
    ...
    execute: function () {
      ...
      _export("LettersConnectorController", LettersConnectorController = (_dec = ccclass('LettersConnectorController'), _dec2 = property({
        ...
        setup(config) {
          var lettersCount = config.length;
          var uiTransform = this.getComponent(UITransform);
          var radiusRatio = 0.72;

          for (var i = 0; i < lettersCount; i++) {
            var letter = config[i];
            var letterNode = instantiate(this.letterNode);
            letterNode.parent = this.node;
            var connectorLetter = letterNode.getComponent(_crd && ConnectorLetterComponent === void 0 ? (_reportPossibleCrUseOfConnectorLetterComponent({
              error: Error()
            }), ConnectorLetterComponent) : ConnectorLetterComponent);
            connectorLetter.setLetter(letter);
            var x = Math.sin(2 * Math.PI / lettersCount * i);
            var y = Math.cos(2 * Math.PI / lettersCount * i);
            var radius = uiTransform.width / 2;
        ...
        onTouch(event) {
          ...
          var touchLocalPos = this.getComponent(UITransform).convertToNodeSpaceAR(new Vec3(touchLocation.x, touchLocation.y, 0));
          touchLocation = new Vec2(touchLocalPos.x, touchLocalPos.y);

          for (var i = 0; i < this.letterNodes.length; i++) {
            var letterNode = this.letterNodes[i];
            ...
            if (!connectorLetter.selected) {
              ...
              var selectedLettersCount = this.selectedLetters.length;
        ...
        onEndTouch(event) {
          var letters = this.getCurrentLetters();
        ...
        redrawLine(touchPosition) {
          ...
          if (this.selectedLetters.length > 0) {
            this.selectedLetters.forEach(letterNode => {
              var pos = letterNode.getPosition();
        ...
        playSound(step) {
          var audioClip = this.sound.letter1;
...
//# sourceMappingURL=b6fc750633f40219e962e8d35a4ac4c0f96a7618.js.map